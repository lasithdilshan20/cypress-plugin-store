...
Cypress.Commands.add('storeValue', (element, variableName) => {
  ...
  if (element.includes('/')) {
    ...
    cy.readFile(filename, {
        log : false
      }).then((json) => {cy.xpath(element).invoke('text').then(($el) => {
                json[variableName] = $el;
                if ($el === '') {
                  delete json[variableName];
                  cy.xpath(element).invoke('val').then(
                      ($el) => { json[variableName] = $el; })
                }
    ...
    cy.readFile(filename, {
        log : false
      }).then((json) => {cy.get(element).invoke('text').then(($el) => {
                json[variableName] = $el;
                if ($el === '') {
                  delete json[variableName];
                  cy.get(element).invoke('val').then(
                      ($el) => { json[variableName] = $el; })
                }
                cy.writeFile(filename, json, {log : false});
              })})
...
// Javascript/typescript function to get the value of a variable from a json
// file
cy.readFile('src/variables.json', {log : false})
    .its(variableName)
    .then((value) => {// Use the value here
                      cy.log(value)})
...
after(() => {
  ...
  cy.log('Out spec ' + x);
    ...
    cy.retrieveValue('lastName')
        .then((x) => { cy.log('lastName Out spec ' + x); })
    cy.retrieveValue('city').then((x) => { cy.log('city Out spec ' + x); })
    cy.retrieveValue('owner').then((x) => { cy.log('owner Out spec ' + x); })
    cy.retrieveValue('firstNameXpath')
        .then((x) => { cy.log('firstNameXpath Out spec ' + x); })
    cy.retrieveValue('cityXpath')
        .then((x) => { cy.log('cityXpath Out spec ' + x); })
